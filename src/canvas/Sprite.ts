...
export class Sprite extends InitComponent() {
  ...
  static render(ctx: WebGL, sprite: Sprite, transform: Float32Array, frame_x: number, frame_y: number) {
    ...
    if (mesh instanceof SpriteMesh && shader instanceof SpriteShader && texture) {
      ...
      gl.uniform2f(shader.location.Frame, frame_x * mesh.uv_width, frame_y * mesh.uv_height);
