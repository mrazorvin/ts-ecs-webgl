...
import { EntityRef, LoopInfo, q, RafScheduler, sys, World } from "@mr/ecs/World";
...
const resize_system = sys([WebGL, Screen, Input], (_, ctx, screen, input) => {
  ...
  ctx.create_context(MAIN_CONTEXT, { width, height, layers: 2 }, Context.create);
...
main_world.system_once(
  sys([WebGL, Screen, Input, CollisionWorld], async (world, ctx, screen, input, sscd) => {
    ...
    const attack_texture = ctx.create_texture(Texture.create, attack_image, {});
    const attack_frame = {
      uv_width: 1,
      uv_height: 1,
      x: 0,
    ...
    const joystick_handle_texture = ctx.create_texture(Texture.create, joystick_handler_image, {});
    main_world.entity([
      Sprite.create(
        world,
        SPRITE_SHADER,
    ...
    const joystick_texture = ctx.create_texture(Texture.create, joystick_image, {});
    ...
    const hero_texture = ctx.create_texture(Texture.create, hero_image, { normals_image: hero_n_image });
    ...
    for (const monster of monsters) {
      ...
      const monster_texture = ctx.create_texture(Texture.create, monster_image, {});
...
main_world.system(
  sys([WebGL], (world, ctx) => {
    ...
    const collect_entities = (_: unknown, transform: Transform, sprite: Sprite, e_type: unknown) => {
