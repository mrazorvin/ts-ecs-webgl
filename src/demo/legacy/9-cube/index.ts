...
main_world.system_once(
	sys([WebGL, Screen], async (world, ctx) => {
		...
		const image = await Texture.load_image(quad_texture_png)!;
		const texture = ctx.create_texture(image, (gl, image) => Texture.create(gl, image, image));

		main_world.entity([Transform3D.create(world), Cube.create(world, texture.id)]);
	}),
);
...
main_world.system(
	sys([WebGL, Camera3D], (world, ctx, camera) => {
		...
		q.run(world, q.id("axes") ?? q([Transform3D, Grid]), (_, transform) => {
			...
			q.run(world, q.id("sprite") ?? q([Transform3D, Cube]), (_, transform, { texture }) => {
				const shader = ctx.shaders.get(CUBE_SHADER);
				const mesh = ctx.meshes.get(CUBE_MESH);

				if (mesh instanceof CubeMesh && shader instanceof CubeShader) {
					const gl_text = ctx.textures.get(texture);
					...
					shader.info.use(
						...
						(gl) => {
							...
							gl.bindTexture(gl.TEXTURE_2D_ARRAY, gl_text?.texture!);
						},
					);
				}
			});
