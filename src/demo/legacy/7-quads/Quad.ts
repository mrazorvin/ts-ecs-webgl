...
export const fragment = `#version 300 es
	...
	out vec4 o_Color;
	void main(void) {
		float color = (in_UV.x <= 0.1 || in_UV.x >= 0.9 || in_UV.y <= 0.1 || in_UV.y >= 0.9) ? 0.0 : 1.0;
		o_Color = vec4(color, color, color, 0.5);
	}
`;
...
export class Quad extends InitComponent({ use_pool: false }) {
	static create = ComponentFactory(Quad, (prev) => prev ?? new Quad());
}
