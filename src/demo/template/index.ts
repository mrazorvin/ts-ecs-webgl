...
export class MainShader extends Shader {
	u_WorldTransform!: WebGLUniformLocation;
	u_SpriteTransform!: WebGLUniformLocation;
	...
	static create(gl: WebGL2RenderingContext) {
		const { program } = t.program(
			...
			{
				layout_attributes: {
					[ShaderGlobals.Attributes.Position]: ShaderGlobals.a_Position,
					[ShaderGlobals.Attributes.Transform]: ShaderGlobals.a_Transformation,
				},
			},
		);
		...
		shader.u_WorldTransform = gl.getUniformLocation(
			program,
			"u_WorldTransform",
		)!;
		shader.u_SpriteTransform = gl.getUniformLocation(
			program,
			"u_SpriteTransform",
		)!;
		gl.useProgram(null);
	...
	static render(
		...
		if (mesh instanceof MainMesh && shader instanceof MainShader) {
			...
			gl.uniformMatrix3fv(shader.u_WorldTransform, false, world_transform);
			gl.uniformMatrix3fv(shader.u_SpriteTransform, false, sprite_transform);
